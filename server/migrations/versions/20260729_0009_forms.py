...
def _rls(table: str) -> None:
    ...
    for operation, clause in (("select", "USING"), ("insert", "WITH CHECK"), ("delete", "USING")):
        op.execute(
            f"CREATE POLICY {table}_{operation} ON {table} FOR {operation.upper()} TO slotera_app {clause} (workspace_id = public.slotera_current_workspace_id())"
        )
    op.execute(
        f"CREATE POLICY {table}_update ON {table} FOR UPDATE TO slotera_app USING (workspace_id = public.slotera_current_workspace_id()) WITH CHECK (workspace_id = public.slotera_current_workspace_id())"
    )
...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column(
            "required_before_payment", sa.Boolean(), server_default=sa.text("true"), nullable=False
        ),
        sa.Column(
        ...
        sa.UniqueConstraint("workspace_id", "id", name="uq_form_templates_workspace_id_id"),
    )
    ...
    op.create_table(
        ...
        sa.PrimaryKeyConstraint("workspace_id", "form_template_id", "service_id"),
    )
    _rls("form_templates")
